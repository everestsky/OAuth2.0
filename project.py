...
def fbconnect():
    ...
    # Exchange to get long time token
...
def gconnect():
    ...
    # See if the user exsits, if not, make a new one-time-use
    user_id = getUserID(login_session['email'])
    if not user_id:
        user_id = createUser(login_session)
    login_session['user_id'] = user_id
...
def gdisconnect():
    ...
    if result['status'] == '200':
        # Reset the user's seesion
        # Because of /disconnect function, delete these same code here
        ...
        # For what reason, the given token was invaild.
